...
mixin template TData()
{
	...
	import flow.lib.vibe.data.serialization, flow.lib.vibe.data.json;
	import flow.base.event, flow.base.type, flow.base.data;
	import flow.interfaces;
	...
	shared static this()
	{
		static if(fqn!(typeof(super)) != "flow.base.data.Data")
	...
	if ((is(T : IData) || isScalarType!T || is(T == UUID) || is(T == SysTime) || is(T == DateTime) || (isArray!T && isScalarType!(ElementType!T))))
{
	...
	import std.traits, std.array, std.conv;
	import flow.lib.vibe.data.json, flow.lib.vibe.data.serialization;
	import flow.base.event, flow.base.type, flow.base.data;
	import flow.interfaces;
