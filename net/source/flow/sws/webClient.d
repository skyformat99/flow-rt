...
module flow.sws.webClient; 

import flow.sws.webServer, flow.sws.webRequest, flow.sws.websocketFrame;
import flow.sws.webParser, flow.sws.webParserWsFrame, flow.sws.webParserFormData, flow.sws.webParserHeader, flow.sws.webParserMultipart;
...
class WebClient : Thread {
				...
				if(!getContent && !isWebsocket) { //if i parse the headers
					...
					if(ret) {
						...
						if("Content-Length" in r.requestHeader) {
							rawData = parseContent(r, rawData);
							r.rawData = rawData;
						}
	...
	private string parseContent(WebRequest r, string rawData) {
		...
		while (rawData.length < maxContent && (bytesRead = currSock.receive(buff)) > 0) {
			rawData ~= cast(string) buff[0..bytesRead].idup;
			
			auto readLen = rawData.length;
