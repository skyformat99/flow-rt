 ...
 */
module flow.core.sync.mutex;


public import core.sync.exception, core.thread;
    ...
    Object.Monitor
{
    ...
    private ThreadID _lockedBy;
    final @nogc nothrow @trusted @property ThreadID lockedBy() {return this._lockedBy;}

    private void setLockedBy(this Q)() nothrow @trusted @nogc
        if (is(Q == Mutex) || is(Q == shared Mutex))
    {
        try {
            this._lockedBy = Thread.getThis.id;
        } catch(Exception ex) {
            SyncError syncErr = cast(SyncError) cast(void*) typeid(SyncError).initializer;
            syncErr.msg = "Unable to set locking thread.";
            throw syncErr;
        }
    }
        ...
        if (is(Q == Mutex) || is(Q == shared Mutex))
    {
        version (Windows)
        {
            if(TryEnterCriticalSection(&m_hndl) != 0) {
                this.setLockedBy();
                return true;
            } else return false; 
        }
        ...
        {
            if(pthread_mutex_trylock(&m_hndl) == 0) {
                this.setLockedBy();
                return true;
            } else return false;
        }
