...
/*private void msgPackHandler(T)(T data, ref Packer p) if(is(T : Data)) {
    if(data !is null) {
        foreach(prop; data.properties)
            prop.as!PropertyInfo.msgPack(data, p);
    }
...
ubyte[] bin(T)(T data, BinarySerializer serializer = BinarySerializer.MsgPack) {
    switch(serializer) {
        case BinarySerializer.MsgPack:
            return data.pack;
        default:
            throw new NotImplementedError;
    }
