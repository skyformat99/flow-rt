...
unittest {
  ...
  auto ex = collectExceptioRef!TTransportException(zlib.write([6]));
  enforce(ex && ex.type == TTransportException.Type.BAD_ARGS);

  ex = collectExceptioRef!TTransportException(zlib.flush());
  enforce(ex && ex.type == TTransportException.Type.BAD_ARGS);

  ex = collectExceptioRef!TTransportException(zlib.finish());
  enforce(ex && ex.type == TTransportException.Type.BAD_ARGS);
}
...
unittest {
  ...
  void testCorrupted(const(ubyte)[] corruptedData) {
    ...
    auto ex = collectExceptioRef!TTransportException(reader.verifyChecksum());
