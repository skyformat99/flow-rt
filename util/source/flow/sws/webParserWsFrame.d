...
module flow.sws.webParserWsFrame; 

import flow.sws.webParser; 

import std.socket, core.thread, std.stdio, std.array, std.string, std.conv;
import std.datetime, std.uuid, std.json, std.uri, std.algorithm;

import flow.sws.websocketFrame;
...
class WebParserWsFrame : WebParser {
	...
	override public bool parse() {
		...
		while(data.length > 2) {
			...
			long availableLength = min(frame.length - frame.payloadData.length, data.length);
