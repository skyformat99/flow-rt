...
module flow.sws.webRequest;

import std.socket;
import flow.sws.webServer, flow.sws.websocketFrame;
import std.variant;
...
class WebRequest {
	...
	private string[int] status_message;
	private string[string] mime;
	 ...
	 * @return bool 
	 */
	bool matchUrl(string rule, ref string[] param) {
		auto r = regex(rule, "i"); 
		
		if(match(this._url, r)) {
			
			auto m = match(this._url, r);
	 ...
	 * @return bool 
	 */
	bool matchUrl(string rule) {
		string[] p;
		return matchUrl(rule, p);
	}
