    ...
    Flow.Service = class Service {
        ...
        _init() {
            if(!this._running && this.getSession())
            {
                ...
                this.beginListen("flow.signals.Pong", function(s) {
                    var flow = Flow;
                ...
                this.addListenSource("flow.signals.Pong", "*");

                this.beginListen("flow.signals.TraceSend", function(s) {
                    console.log("flow.signals.TraceSend:" + JSON.stringify(s));

                    Flow.tick.trace.addOrUpdateNode(s.source.id, s.data, s.data.type, "dot", 0.5);
                });
                this.addListenSource("flow.signals.TraceSend", "*");

                this.beginListen("flow.signals.TraceReceive", function(s) {
                    console.log("flow.signals.TraceReceive:" + JSON.stringify(s));

                    Flow.tick.trace.addOrUpdateNode(s.source.id, s.data, s.data.type, "dot", 0.5);
                });
                this.addListenSource("flow.signals.TraceReceive", "*");

                this.beginListen("flow.signals.TraceBeginTick", function(s) {
                    console.log("flow.signals.TraceBeginTick:" + JSON.stringify(s));

                    Flow.tick.trace.addOrUpdateNode(s.source.id, s.data, Flow.tick.trace.renderTick(s.data), "box", 1);
                });
                this.addListenSource("flow.signals.TraceBeginTick", "*");

                this.beginListen("flow.signals.TraceEndTick", function(s) {
                    console.log("flow.signals.TraceEndTick:" + JSON.stringify(s));
                    
                    Flow.tick.trace.addOrUpdateNode(s.source.id, s.data, Flow.tick.trace.renderTick(s.data), "box", 1);
                });
                this.addListenSource("flow.signals.TraceEndTick", "*");
        ...
        _refresh() {
            ...
            var s = {dataType: "flow.signals.Ping"};
            Flow.service.send(s);
        }
